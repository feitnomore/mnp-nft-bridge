...
NAD_INITIAL_FILE="${SCENARIO_DIR}/00_nad_bridge.yaml"
POD_FILE="${SCENARIO_DIR}/01_pod_on_nad_config_update.yaml"
MNP_FILE="${SCENARIO_DIR}/02_mnp_on_nad_config_update.yaml"
NAD_UPDATED_FILE="${SCENARIO_DIR}/03_nad_updated_config.yaml"

TARGET_POD_NAME="pod-on-nad-config-update"
POD_NAMESPACE="default" 
MNP_NAME="policy-on-nad-config-update" 
...
cleanup() {
    ...
    # Delete using the name, as the file may have been overwritten
    ${KUBECTL} delete --ignore-not-found=true -f "${NAD_UPDATED_FILE}"
    ${KUBECTL} delete --ignore-not-found=true -f "${NAD_INITIAL_FILE}"
...
# 1. Initial Setup: Apply Initial NetworkAttachmentDefinition, Pod and MultiNetworkPolicy
log_info "🔩 Applying Initial NetworkAttachmentDefinition: ${NAD_INITIAL_FILE}"
${KUBECTL} apply -f "${NAD_INITIAL_FILE}"
...
EXPECTED_CIDR="192.168.100.90/32" 
EXPECTED_PORT="80"              
...
# 2. Action: Update the NetworkAttachmentDefinition configuration (without changing the type: "bridge")
log_info "🔩 Updating NetworkAttachmentDefinition ${NAD_NAMESPACE_IN_MANIFEST}/${NAD_NAME_IN_MANIFEST} with new config: ${NAD_UPDATED_FILE}"
${KUBECTL} apply -f "${NAD_UPDATED_FILE}"

log_info "⏳ Waiting for controller to process NAD update (30 seconds)..."
sleep 30

# 3. NetworkAttachmentDefinition Post-Update Checks
log_info "🔎 === Checking Status After NetworkAttachmentDefinition Config Update ==="
log_info "🔎 [Post-NetworkAttachmentDefinition Update] Checking if mnp-nft-bridge controller is still running..."
...
# since the NetworkAttachmentDefinition driver type ("bridge") has not changed.
log_info "🔎 [Post-NetworkAttachmentDefinition Update] Checking if JUMP Ingress rule for MAC ${TARGET_POD_MAC_GLOBAL} to ${EXPECTED_INGRESS_CHAIN_NAME} STILL EXISTS..."
JUMP_RULE_INGRESS_POST_NAD_UPDATE=$(${EXEC_KIND} nft list chain bridge filter KUBE_MULTI_INGRESS | \
    grep -F "ether daddr ${TARGET_POD_MAC_GLOBAL} jump ${EXPECTED_INGRESS_CHAIN_NAME}" || echo "NOT_FOUND")
if [[ "$JUMP_RULE_INGRESS_POST_NAD_UPDATE" == "NOT_FOUND" ]]
then
    log_error "❌ [Post-NetworkAttachmentDefinition Update] JUMP Ingress rule for MAC ${TARGET_POD_MAC_GLOBAL} for ${EXPECTED_INGRESS_CHAIN_NAME} NOT found, but was expected."
    ${EXEC_KIND} nft list chain bridge filter KUBE_MULTI_INGRESS
    exit 1
fi
log_success "✔️ [Post-NetworkAttachmentDefinition Update] JUMP Ingress rule still exists, as expected."

log_info "🔎 [Post-NetworkAttachmentDefinition Update] Checking if chain ${EXPECTED_INGRESS_CHAIN_NAME} STILL EXISTS..."
if ! ${EXEC_KIND} nft list chain bridge filter "${EXPECTED_INGRESS_CHAIN_NAME}" > /dev/null 2>&1; then
    log_error "❌ [Post-NetworkAttachmentDefinition Update] Chain Ingress ${EXPECTED_INGRESS_CHAIN_NAME} NOT found, but was expected."
    exit 1
fi
log_success "✔️ [Post-NetworkAttachmentDefinition Update] Chain Ingress ${EXPECTED_INGRESS_CHAIN_NAME} still exists, as expected."

log_info "🔎 [Post-NetworkAttachmentDefinition Update] Checking if rules within chain ${EXPECTED_INGRESS_CHAIN_NAME} STILL EXIST..."
INGRESS_POD_CHAIN_RULES_POST_NAD_UPDATE=$(${EXEC_KIND} nft list chain bridge filter "${EXPECTED_INGRESS_CHAIN_NAME}")
RULE_LINE_POST_NAD_UPDATE=$(echo "$INGRESS_POD_CHAIN_RULES_POST_NAD_UPDATE" | grep "ip saddr @mnp-src-ip-" | grep "tcp dport @mnp-tcp-port-" | grep "accept" | head -n1)
if [[ -z "$RULE_LINE_POST_NAD_UPDATE" ]]
then
    log_error "❌ [Post-NetworkAttachmentDefinition Update] Ingress Rule (IP Set -> TCP Port Set) NOT found in chain ${EXPECTED_INGRESS_CHAIN_NAME}, but was expected."
    echo "$INGRESS_POD_CHAIN_RULES_POST_NAD_UPDATE"
    exit 1
fi
log_success "✔️ [Post-NetworkAttachmentDefinition Update] Ingress rule on pod specific chain still exists as expected."
...
then
    log_info "🔎 [Post-NetworkAttachmentDefinition Update] Checking NftPodChainMetadataCache for ${TARGET_POD_NAME}..."
    ...
    then
        log_error "❌ [Post-NetworkAttachmentDefinition Update] Cache Ingress entry for ${EXPECTED_CACHE_KEY_INGRESS} not found or not recently updated as expected."
        ...
        log_success "✔️ [Post-NetworkAttachmentDefinition Update] Cache Ingress entry for ${EXPECTED_CACHE_KEY_INGRESS} found (or recently updated)."
    fi
else
    log_warn "⚠️ [Post-NetworkAttachmentDefinition Update] Unable to verify NftPodChainMetadataCache."
fi
